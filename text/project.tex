...
Convolutional neural netwoks (CNN) are deep learning neaural networks suitable for image classificatigeon. Deep learning algorithms are a class of machine learning algorithms that uses multiple layers to extract higher level features. The main reason to prefer deep learning techniques is that many layers with fewer weights are more flexible and precise in classification than neural networs with many weights and fewer layers.    
Convolution layer
The pooling layer just extract a reduced representation of the data computed in the convlutional layer. The dimension specified in the pooling layer defines a dimensions of the matrix of pixels to look at, once the matrix is identified it can be reduce by extracting the hight value in it. Notice that the mean or min value can also be extracted.
After this process the image dimensions has been reduced. After the convolutional part of the net there exist some fully connected layer that returns a prediction from the featurtes extracted from the convolutional section. The output layer has one neuron for each possible class and the class having the highest probability will be chose as a prediction.
The following images represent the training accuracy of the cnn used to classify the data. The training accuracy reach as high as 0.99 while the test accuracy is a bit lower.
THe test accuracy reached when using grayscale features is 0.90, while the test accuracy when using rgb features is 0.94.
\begin{figure}[H]  
  \begin{adjustwidth}{-4cm}{-4cm}
     \subfloat{%
...
The CNN seem to be the best model to use to classify images.
\section{Conclusions}
RISULTATI MIGLIORI
